...
def calculate_rhythm_similarities(video_id=None, second_video_id=None, all_features=None):
    if video_id is not None and second_video_id is not None:
        ...
        for second_video_id in all_features:
            if second_video_id != video_id:
                similarities[second_video_id] = compute_rhythm_similarities(
                    first_feat, all_features[second_video_id])
        return similarities
        ...
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(calculate_rhythm_similarities, video_id,
                                       all_features=all_features): video_id for video_id in all_features}
