...
def get_video_features(
    preprocess, model, features=None, video_folder=None, video_paths=None
):
    ...
    for path in tqdm(video_paths, ncols=70):
        ...
        frames = (
            torch.from_numpy(preprocess(path)).permute(0, 4, 1, 2, 3).float().cuda()
        )
    ...
    normed_features = {
        vid: model.normalize_features(feats.cuda())
        for vid, feats in all_features.items()
    }
